...
public class Cliente {
	...
	private static class PalThread implements Runnable {
		...
		public void run() {
			try {
				...
				global[i] = worker.encuentraPrimos(min, max);
				long t2 = System.currentTimeMillis();
				System.out.println("Soy" + i + "y he terminado en : "+ ((t2-t1) /1000));
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (r != 0) {
				t = new Thread[n + 1];
				global = (ArrayList<Integer>[]) new ArrayList[n + 1];
			} else {
				t = new Thread[n];
				global = (ArrayList<Integer>[]) new ArrayList[n];
			}

			for (int i = 0; i < n; i++) {
				t[i] = new Thread(new PalThread(workers.get(i), min + q * i,
						min + q * i + q, i));
			}
			if (r != 0)
				t[n] = new Thread(
						new PalThread(workers.get(0), max - r, max, n));
	...
	private static void mostrarPrimos(ArrayList<Integer>[] primos) {
